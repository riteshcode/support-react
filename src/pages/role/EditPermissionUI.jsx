...
import { SetKeyValue } from "config/StaticKey";

const EditPermissionUI = (props) => {
  const { register, setValue } = useFormContext();
  const { checkedPermission } = props;
  ...
  const setPerSelectItem = () => {
    if (checkedPermission !== undefined) {
      if (checkedPermission !== "" && checkedPermission.length > 0) {
        for (let index = 0; index < checkedPermission.length; index++) {
          const fieldName = `permission.${checkedPermission[index]["section_id"]}.${checkedPermission[index]["permissions_ids"]}`;
          setValue(fieldName, checkedPermission[index]["permission_types_id"]);
        }
      }
    }
  };
...
export default EditPermissionUI;
