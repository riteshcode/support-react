...
import { QuotationUrl, languageDeleteUrl, QuotationStatusUrl } from "config";
...
function Index() {
  ...
  const ChangeFunction = (quoteId, statusId) => {
    ...
    POST(QuotationStatusUrl, editData)
      .then((response) => {
        const { message } = response.data;
        filterItem("refresh", "", "");
        Notify(true, Trans(message, language));
    ...
    <Content>
      <PageHeader
        breadcumbs={[
          ...
          { title: Trans("QUOTATION", language), link: "", class: "active" },
        ]}
      />
      <div className="row row-xs">
        <div className="col-sm-12 col-lg-12">
          <CheckPermission PageAccess={PageQuatation} PageAction={PreView}>
            <div className="card" id="custom-user-list">
              <div className="card-header pd-y-15 pd-x-20 d-flex align-items-center justify-content-between">
                <h6 className="tx-uppercase tx-semibold mg-b-0">
                  {Trans("QUOTATION_LIST", language)}
                </h6>
                <div className="d-none d-md-flex">
                  <CheckPermission
                    PageAccess={PageQuatation}
                    PageAction={PreAdd}
                  >
                    <Anchor
                      className="btn btn-primary pd-3"
                      path={WebsiteLink("/quotation/create")}
                    >
                      <FeatherIcon
                        icon="plus"
                        fill="white"
                ...
                {contentloadingStatus ? (
                  ...
                  <div className="row">
                    <Table
                      ...
                      dataList={dataList}
                      pageName={PageQuatation} // for checkpermission
                      sortBy={sortByS}
                      orderBy={orderByS}
                      filterItem={filterItem}
                      editFun={editFun}
