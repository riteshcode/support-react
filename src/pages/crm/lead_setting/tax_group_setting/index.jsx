...
function Index() {
  ...
  const [sortByS, SetsortByS] = useState("tax_group_id");
  ...
  const showColumn = [
    { label: Trans("SL_NO", language), field: "sl_no", sort: true },
    {
      ...
      field: "tax_group_name",
      sort: true,
    },
    {
      label: Trans("TAX_LIST", language),
      field: "tax_list",
      sort: false,
    },
  ...
  const getData = (pagev, perPageItemv, searchData, sortBys, OrderBy) => {
    ...
    POST(TaxGroupSettingUrl, filterData)
      .then((response) => {
  ...
  const ChangeFunction = (tax_group_id) => {
    const editData = {
      api_token: apiToken,
      tax_group_id: tax_group_id,
    };
    POST(TaxGroupSettingStatusUrl, editData)
      .then((response) => {
        const { message } = response.data;
        filterItem("refresh", "", "");
        Notify(true, Trans(message, language));
    ...
    <Content>
      <PageHeader
        breadcumbs={[
          ...
          { title: Trans("TAX_GROUP", language), link: "", class: "active" },
        ]}
      />
      ...
      <Modal show={show} onHide={handleModalClose}>
        <Modal.Header>
          <Modal.Title>{Trans("ADD_TAX_GROUP", language)}</Modal.Title>
      ...
      <Modal show={editModalShow} onHide={handleEditModalClose}>
        <Modal.Header>
          <Modal.Title>{Trans("UPDATE_TAX_GROUP", language)}</Modal.Title>
