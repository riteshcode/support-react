...
function Index() {
  ...
  const [sortByS, SetsortByS] = useState("source_id");
  ...
  const showColumn = [
    ...
    { label: Trans("SOURCE_NAME", language), field: "source_name", sort: true },
  ...
  const ChangeFunction = (source_id) => {
    const editData = {
      api_token: apiToken,
      source_id: source_id,
    };
    POST(leadSourceStatusUrl, editData)
      .then((response) => {
  ...
  const deleteFun = (deleteId) => {
    ...
    POST(leadSourceDeleteUrl, editData)
      .then((response) => {
        const { message } = response.data;
        filterItem("refresh", "", "");
        Notify(true, Trans(message, language));
    ...
    <Content>
      <PageHeader
        breadcumbs={[
          ...
          { title: Trans("LEAD_SOURCE", language), link: "", class: "active" },
        ]}
        heading={Trans("LEAD_SOURCE_LIST", language)}
      />
            ...
            <div className="card" id="custom-user-list">
              <div className="card-header pd-y-15 pd-x-20 d-flex align-items-center justify-content-between">
                <h6 className="tx-uppercase tx-semibold mg-b-0">
                  {Trans("LEAD_SOURCE_LIST", language)}
                </h6>
      ...
      <Modal show={show} onHide={handleModalClose}>
        <Modal.Header>
          <Modal.Title>{Trans("ADD_LEAD_SOURCE", language)}</Modal.Title>
      ...
      <Modal show={editModalShow} onHide={handleEditModalClose}>
        <Modal.Header>
          <Modal.Title>{Trans("UPDATE_LEAD_SOURCE", language)}</Modal.Title>
