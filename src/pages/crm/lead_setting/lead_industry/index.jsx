...
import {
  leadIndustryUrl,
  leadIndustryStatusUrl,
  leadIndustryDeleteUrl,
} from "config";
import {
  PageDepartment,
  PreAdd,
...
function Index() {
  ...
  const [sortByS, SetsortByS] = useState("industry_id");
  ...
  const getUser = (pagev, perPageItemv, searchData, sortBys, OrderBy) => {
    ...
    POST(leadIndustryUrl, filterData)
      .then((response) => {
  ...
  const ChangeFunction = (industry_id) => {
    ...
    POST(leadIndustryStatusUrl, editData)
      .then((response) => {
  ...
  const deleteFun = (deleteId) => {
    ...
    POST(leadIndustryDeleteUrl, editData)
      .then((response) => {
        const { message } = response.data;
        filterItem("refresh", "", "");
        Notify(true, Trans(message, language));
    ...
    <Content>
      <PageHeader
        breadcumbs={[
          ...
          {
            title: Trans("LEAD_INDUSTRY", language),
            link: "",
            class: "active",
          },
        ]}
        heading={Trans("LEAD_INDUSTRY_LIST", language)}
      />
            ...
            <div className="card" id="custom-user-list">
              <div className="card-header pd-y-15 pd-x-20 d-flex align-items-center justify-content-between">
                <h6 className="tx-uppercase tx-semibold mg-b-0">
                  {Trans("LEAD_INDUSTRY_LIST", language)}
                </h6>
      ...
      <Modal show={show} onHide={handleModalClose}>
        <Modal.Header>
          <Modal.Title>{Trans("ADD_LEAD_INDUSTRY", language)}</Modal.Title>
      ...
      <Modal show={editModalShow} onHide={handleEditModalClose}>
        <Modal.Header>
          <Modal.Title>{Trans("UPDATE_LEAD_INDUSTRY", language)}</Modal.Title>
