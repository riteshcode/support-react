...
function Index() {
  ...
  const [sortByS, SetsortByS] = useState("status_id");
  ...
  const showColumn = [
    ...
    {
      label: Trans("STATUS_NAME", language),
      field: "status_name",
      sort: true,
    },
  ...
  const getUser = (pagev, perPageItemv, searchData, sortBys, OrderBy) => {
    ...
    POST(CRMleadStatusUrl, filterData)
      .then((response) => {
  ...
  const ChangeFunction = (status_id) => {
    const editData = {
      api_token: apiToken,
      status_id: status_id,
    };
    POST(leadStatusStatusUrl, editData)
      .then((response) => {
  ...
  const deleteFun = (deleteId) => {
    ...
    POST(leadStatusDeleteUrl, editData)
      .then((response) => {
        const { message } = response.data;
        filterItem("refresh", "", "");
        Notify(true, Trans(message, language));
    ...
    <Content>
      <PageHeader
        breadcumbs={[
          ...
          {
            title: Trans("LEAD_STATUS", language),
            link: "",
            class: "active",
          },
        ]}
        heading={Trans("LEAD_STATUS", language)}
      />
      ...
      <Modal show={show} onHide={handleModalClose}>
        <Modal.Header>
          <Modal.Title>{Trans("ADD_LEAD_STATUS", language)}</Modal.Title>
      ...
      <Modal show={editModalShow} onHide={handleEditModalClose}>
        <Modal.Header>
          <Modal.Title>{Trans("UPDATE_LEAD_STATUS", language)}</Modal.Title>
