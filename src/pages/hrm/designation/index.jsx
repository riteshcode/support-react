...
import { designationUrl, designationDeleteUrl } from "config";
import {
  PageDepartment,
  PreAdd,
  PreView,
...
function Index() {
  ...
  const [sortByS, SetsortByS] = useState("designation_id");
  ...
  const showColumn = [
    ...
    {
      label: Trans("DESIGNATION_NAME", language),
      field: "designation_name",
      sort: true,
    },
    { label: Trans("CREATED_BY", language), field: "created_by", sort: false },
  ...
  const getUser = (pagev, perPageItemv, searchData, sortBys, OrderBy) => {
    ...
    POST(designationUrl, filterData)
      .then((response) => {
  ...
  const deleteFun = (deleteId) => {
    const editData = {
      api_token: apiToken,
      designation_id: deleteId,
    };
    POST(designationDeleteUrl, editData)
      .then((response) => {
    ...
    <Content>
      <PageHeader
        breadcumbs={[
          ...
          { title: Trans("DESIGNATION", language), link: "", class: "active" },
        ]}
        heading={Trans("DESIGNATION_LIST", language)}
      />
            ...
            <div className="card" id="custom-user-list">
              <div className="card-header pd-y-15 pd-x-20 d-flex align-items-center justify-content-between">
                <h6 className="tx-uppercase tx-semibold mg-b-0">
                  {Trans("DESIGNATION_LIST", language)}
                </h6>
                <div className="d-none d-md-flex">
                  ...
                  >
                    <Button variant="primary" onClick={handleModalShow}>
                      ...
                      />
                      {Trans("CREATE_DESIGNATION", language)}
                    </Button>
                  </CheckPermission>
                </div>
              </div>
      ...
      <Modal show={show} onHide={handleModalClose}>
        <Modal.Header>
          <Modal.Title>{Trans("CREATE_DESIGNATION", language)}</Modal.Title>
      ...
      <Modal show={editModalShow} onHide={handleEditModalClose}>
        <Modal.Header>
          <Modal.Title>{Trans("UPDATE_DESIGNATION", language)}</Modal.Title>
