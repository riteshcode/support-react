...
import ProductType from "./component/Create/ProductType";
import Select from "react-select";
import WebsiteLink from "config/WebsiteLink";
import LangProductInfo from "./component/Create/LangProductInfo";
import ProductAttribute from "./component/Create/ProductAttribute";
import Feature from "./component/Create/Feature";
import BulkPrice from "./component/Create/BulkPrice";
...
const Create = () => {
  ...
  const methods = useForm({
    defaultValues: {
      proFeature: [{ feature_key: "", feature_values: "" }],
      bulkPrice: [{ product_qty: "", product_price: "" }],
    },
  });
  ...
  const onSubmit = (formData) => {
    ...
    POST(productStoreUrl, saveFormData)
      .then((response) => {
    ...
    POST(productCreateUrl, formData)
      .then((response) => {
        ...
        if (status) {
          ...
          SetFeature(data.product_attribute);
  ...
  const HandleDocumentUpload = (event, previewUrlId, StoreID) => {
    ...
    readers.onload = function (e) {
      ...
      document.getElementById(
        previewUrlId
      ).innerHTML = `<a href=${e.target.result} download ><img src=${e.target.result} height="100" width="100" /></a>`;
    };
  ...
  const removeTag = (pos) => {
    ...
    setValue("product_tags", newTags.join(","));
  };
  ...
  const calculateSellprice = () => {
    ...
    const shippingCharge = parseFloat(getValues("shipping_charge"));
    const productDiscountType = getValues("product_discount_type");
    const productDiscountAmount = parseFloat(
      getValues("product_discount_amount")
    );

    if (
      shippingCharge === NaN ||
      productDiscountType === NaN ||
      productDiscountAmount === NaN
    ) {
      return false;
    }

    let extrper = 0;
    if (productDiscountType !== "NO") {
      if (productDiscountAmount) {
        if (productDiscountType === "flat") extrper = productDiscountAmount;
        else {
          extrper = (productDiscountAmount / 100) * sellPrice;
        }
      }
    }
    let ship = 0;
    if (shippingCharge) ship = shippingCharge;

    let sellPriceShow = sellPrice + ship - extrper;
    setValue("product_sale_price_show", sellPriceShow);
  };

  const [manStock, SetManStock] = useState(1);
                            ...
                            <Col col={3}>
                              <FormGroup mb="20px">
                                <Input
                                  ...
                                  defaultValue={0}
                                  readOnly={manStock === "1" ? false : true}
                                />
