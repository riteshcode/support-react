...
import { supplierUrl, supplierStatusUrl, supplierDeleteUrl } from "config";
import {
  PageSupplier,
  PreAdd,
  PreView,
...
function Index() {
  ...
  const [sortByS, SetsortByS] = useState("supplier_id");
  ...
  const showColumn = [
    ...
    { label: Trans("NAME", language), field: "supplier_name", sort: true },
    {
      label: Trans("ADDRESS", language),
      field: "supplier_address",
      sort: false,
    },
    { label: Trans("CITY", language), field: "supplier_city", sort: false },
    { label: Trans("STATE", language), field: "supplier_state", sort: false },
  ...
  const ChangeFunction = (supplier_id) => {
    ...
    POST(supplierStatusUrl, editData)
      .then((response) => {
    ...
    <Content>
      <PageHeader
        breadcumbs={[
          ...
          { title: Trans("SUPPLIER", language), link: "", class: "active" },
        ]}
        heading={Trans("SUPPLIER_LIST", language)}
      />

      <div className="row row-xs">
        <div className="col-sm-12 col-lg-12">
          <CheckPermission PageAccess={PageSupplier} PageAction={PreView}>
            <div className="card" id="custom-user-list">
              <div className="card-header pd-y-15 pd-x-20 d-flex align-items-center justify-content-between">
                <h6 className="tx-uppercase tx-semibold mg-b-0">
                  {Trans("SUPPLIER_LIST", language)}
                </h6>
                <div className="d-none d-md-flex">
                  <CheckPermission
                    PageAccess={PageSupplier}
                    PageAction={PreAdd}
                  >
                ...
                {contentloadingStatus ? (
                  ...
                  <div className="row">
                    <Table
                      ...
                      dataList={userlist}
                      pageName={PageSupplier} // for checkpermission
                      sortBy={sortByS}
                      orderBy={orderByS}
                      filterItem={filterItem}
                      editFun={editFun}
      ...
      <Modal show={show} onHide={handleModalClose}>
        <Modal.Header>
          <Modal.Title>{Trans("ADD_SUPPLIER", language)}</Modal.Title>
      ...
      <Modal show={editModalShow} onHide={handleEditModalClose}>
        <Modal.Header>
          <Modal.Title>{Trans("UPDATE_SUPPLIER", language)}</Modal.Title>
