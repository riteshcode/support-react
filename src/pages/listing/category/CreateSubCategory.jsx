...
const CreateSubCategory = (props) => {
  ...
  const onSubmit = (formData) => {
    ...
    POST(categoryStoreUrl, saveFormData)
      .then((response) => {
  ...
  const ModuleChange = (e) => {
    ...
    if (value !== undefined || value !== "") {
      ...
      SetSubCategoryListing(JSON.parse(sectionlistData));
    }
  };
            ...
            <FormGroup mb="20px">
              ...
              <select
                ...
                {...register("main_category", {
                  required: Trans("MAIN_CATEGORY_REQUIRED", language),
                })}
                onChange={(e) => {
                  ModuleChange(e);
                }}
              >
                ...
                {categoryList &&
                  categoryList.map((curr) => (
                    <option
                      value={`${curr.categories_id}`}
                      sectionlist={`${JSON.stringify(curr.sub_category)}`}
                      key={curr.categories_id}
                    >
                      {curr.category_name}
                    </option>
                  ))}
            ...
            <Col col={12}>
              <FormGroup mb="20px">
                <Label
                  display="block"
                  mb="5px"
                  htmlFor={Trans("SUB_CATEGORY", language)}
                >
                  {Trans("SUB_CATEGORY", language)}
                </Label>
                <select
                  id={Trans("SUB_CATEGORY", language)}
                  placeholder={Trans("SUB_CATEGORY", language)}
                  className="form-control"
                  {...register("sub_category", {
                    required: Trans("SUB_CATEGORY_REQUIRED", language),
                  })}
                >
                  <option value="">
                    {Trans("SELECT_SUB_CATEGORY", language)}
                  </option>
                  {subCategoryListing &&
                    subCategoryListing.map((curr) => (
                      <option
                        value={curr.categories_id}
                        key={curr.categories_id}
                      >
                ...
                <span className="required">
                  <ErrorMessage errors={errors} name="sub_category" />
                </span>
              </FormGroup>
            </Col>
          )}
                        ...
                        <Col col={6}>
                          <FormGroup mb="20px">
                            <Input
                              ...
                              className="form-control"
                              {...register(`categories_name_${languages_id}`, {
                                required: Trans(
                        ...
                        <Col col={6}>
                          <FormGroup mb="20px">
                            <Input
                              ...
                              className="form-control"
                              {...register(`categories_title_${languages_id}`)}
                            />
                          </FormGroup>
                        </Col>
