...
function Index() {
  ...
  const [sortByS, SetsortByS] = useState("payment_method_id");
  ...
  const showColumn = [
    ...
    {
      label: Trans("PAYMENT_DESCRIPTION", language),
  ...
  const getData = (pagev, perPageItemv, searchData, sortBys, OrderBy) => {
    ...
    POST(paymentMethodUrl, filterData)
      .then((response) => {
    ...
    POST(paymentMethodUrl, filterData2)
      .then((response) => {
        const { status, data, message } = response.data;
        if (status) {
  ...
  const ChangeFunction = (id) => {
    ...
    POST(paymentMethodChangeStatusUrl, editData)
      .then((response) => {
        const { message } = response.data;
        filterItem("refresh", "", "");
        Notify(true, Trans(message, language));
    ...
    <Content>
      <PageHeader
        breadcumbs={[
          ...
          {
            title: Trans("PAYMENT_METHOD", language),
            link: "",
            class: "active",
          },
        ]}
      ...
      <div className="row row-xs">
        <div className="col-sm-12 col-lg-12">
          <CheckPermission PageAccess={PagePaymentMethod} PageAction={PreView}>
            <div className="card" id="custom-user-list">
              <div className="card-header pd-y-15 pd-x-20 d-flex align-items-center justify-content-between">
                <h6 className="tx-uppercase tx-semibold mg-b-0">
                  {Trans("PAYMENT_METHOD_LIST", language)}
                </h6>
                <div className="d-none d-md-flex">
                  <CheckPermission
                    PageAccess={PagePaymentMethod}
                    PageAction={PreAdd}
                  >
                    <Button variant="primary" onClick={handleModalShow}>
                      ...
                      />
                      {Trans("ADD_PAYMENT_METHOD", language)}
                    </Button>
                  </CheckPermission>
                </div>
              </div>
                ...
                {contentloadingStatus ? (
                  ...
                  <div className="row">
                    <Table
                      ...
                      dataList={dataList}
                      pageName={PagePaymentMethod} // for checkpermission
                      sortBy={sortByS}
                      orderBy={orderByS}
      ...
      <Modal show={show} onHide={handleModalClose}>
        <Modal.Header>
          <Modal.Title>{Trans("ADD_PAYMENT_METHOD", language)}</Modal.Title>
      ...
      <Modal show={editModalShow} onHide={handleEditModalClose}>
        <Modal.Header>
          <Modal.Title>{Trans("UPDATE_PAYMENT_METHOD", language)}</Modal.Title>
