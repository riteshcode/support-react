...
function Index() {
  ...
  const getData = (pagev, perPageItemv, searchData, sortBys, OrderBy) => {
    ...
    POST(industryCategoryUrl, filterData)
      .then((response) => {
    ...
    POST(industryCategoryUrl, filterData2)
      .then((response) => {
        const { status, data, message } = response.data;
        if (status) {
  ...
  const ChangeFunction = (industry_category_id) => {
    const editData = {
      api_token: apiToken,
      industry_category_id: industry_category_id,
    };
    POST(industryCategoryStatusUrl, editData)
      .then((response) => {
        const { message } = response.data;
        filterItem("refresh", "", "");
        Notify(true, Trans(message, language));
    ...
    <Content>
      <PageHeader
        breadcumbs={[
          ...
          {
            title: Trans("INDUSTRY_CATEGORY", language),
            link: "",
            class: "active",
          },
        ]}
            ...
            <div className="card" id="custom-user-list">
              <div className="card-header pd-y-15 pd-x-20 d-flex align-items-center justify-content-between">
                <h6 className="tx-uppercase tx-semibold mg-b-0">
                  {Trans("INDUSTRY_CATEGORY_LIST", language)}
                </h6>
                <div className="d-none d-md-flex">
                  ...
                  >
                    <Button variant="primary" onClick={handleModalShow}>
                      ...
                      />
                      {Trans("ADD_INDUSTRY_CATEGORY", language)}
                    </Button>
                  </CheckPermission>
                </div>
              </div>
      ...
      <Modal show={show} onHide={handleModalClose}>
        <Modal.Header>
          <Modal.Title>{Trans("ADD_INDUSTRY_CATEGORY", language)}</Modal.Title>
      ...
      <Modal show={editModalShow} onHide={handleEditModalClose}>
        <Modal.Header>
          <Modal.Title>
            {Trans("UPDATE_INDUSTRY_CATEGORY", language)}
          </Modal.Title>
