...
function Index() {
  ...
  const [sortByS, SetsortByS] = useState("currencies_name");
  ...
  const showColumn = [
    { label: Trans("SL_NO", language), field: "sl_no", sort: true },
    {
      ...
      field: "currencies_name",
      sort: true,
    },
    {
      label: Trans("CURRENCY_CODE", language),
      field: "currencies_code",
      sort: false,
    },
    {
      label: Trans("DECIMAL_PLACE", language),
      field: "decimal_places",
      sort: false,
    },
    { label: Trans("value", language), field: "value", sort: false },
  ...
  const ChangeFunction = (id) => {
    ...
    POST(currencyChangeStatusUrl, editData)
      .then((response) => {
        const { message } = response.data;
        filterItem("refresh", "", "");
        Notify(true, Trans(message, language));
    ...
    <Content>
      <PageHeader
        breadcumbs={[
          ...
          { title: Trans("CURRENCY", language), link: "", class: "active" },
        ]}
      />
      <div className="row row-xs">
        <div className="col-sm-12 col-lg-12">
          <CheckPermission PageAccess={PageCurrency} PageAction={PreView}>
            <div className="card" id="custom-user-list">
              <div className="card-header pd-y-15 pd-x-20 d-flex align-items-center justify-content-between">
                <h6 className="tx-uppercase tx-semibold mg-b-0">
                  {Trans("CURRENCY_LIST", language)}
                </h6>
                <div className="d-none d-md-flex">
                  <CheckPermission
                    PageAccess={PageCurrency}
                    PageAction={PreAdd}
                  >
                ...
                {contentloadingStatus ? (
                  ...
                  <div className="row">
                    <Table
                      ...
                      dataList={dataList}
                      pageName={PageCurrency} // for checkpermission
                      sortBy={sortByS}
      ...
      <Modal show={show} onHide={handleModalClose}>
        <Modal.Header>
          <Modal.Title>{Trans("ADD_CURRENCY", language)}</Modal.Title>
      ...
      <Modal show={editModalShow} onHide={handleEditModalClose}>
        <Modal.Header>
          <Modal.Title>{Trans("UPDATE_CURRENCY", language)}</Modal.Title>
