...
function Index() {
  ...
  const getData = (pagev, perPageItemv, searchData, sortBys, OrderBy) => {
    ...
    POST(AppSettingGroupUrl, filterData)
      .then((response) => {
    ...
    POST(AppSettingGroupUrl, filterData2)
      .then((response) => {
        const { status, data, message } = response.data;
        if (status) {
  ...
  const ChangeFunction = (id) => {
    ...
    POST(AppSettingGroupStatusUrl, editData)
      .then((response) => {
        const { message } = response.data;
        filterItem("refresh", "", "");
        Notify(true, Trans(message, language));
    ...
    <Content>
      <PageHeader
        breadcumbs={[
          ...
          {
            title: Trans("SETTING_GROUP", language),
            link: "",
            class: "active",
          },
        ]}
            ...
            <div className="card" id="custom-user-list">
              <div className="card-header pd-y-15 pd-x-20 d-flex align-items-center justify-content-between">
                <h6 className="tx-uppercase tx-semibold mg-b-0">
                  {Trans("SETTING_GROUP_LIST", language)}
                </h6>
                <div className="d-none d-md-flex">
                  ...
                  >
                    <Button variant="primary" onClick={handleModalShow}>
                      ...
                      />
                      {Trans("ADD_SETTING_GROUP", language)}
                    </Button>
                  </CheckPermission>
                </div>
              </div>
