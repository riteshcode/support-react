...
function Index() {
  ...
  const [sortByS, SetsortByS] = useState("group_id");
  ...
  const getData = (pagev, perPageItemv, searchData, sortBys, OrderBy) => {
    ...
    POST(AppSettingUrl, filterData2)
      .then((response) => {
        const { status, data, message } = response.data;
        if (status) {
  ...
  const ChangeFunction = (id) => {
    ...
    POST(AppSettingStatusUrl, editData)
      .then((response) => {
        const { message } = response.data;
        filterItem("refresh", "", "");
        Notify(true, Trans(message, language));
    ...
    <Content>
      <PageHeader
        breadcumbs={[
          ...
          {
            title: Trans("SETTING_GROUP_KEY", language),
            link: "",
            class: "active",
          },
        ]}
      ...
      <div className="row row-xs">
            ...
            <div className="card" id="custom-user-list">
              <div className="card-header pd-y-15 pd-x-20 d-flex align-items-center justify-content-between">
                <h6 className="tx-uppercase tx-semibold mg-b-0">
                  {Trans("SETTING_GROUP_KEY_LIST", language)}
                </h6>
        ...
        <Modal.Header>
          <Modal.Title>{Trans("ADD_SETTING_GROUP_KEY", language)}</Modal.Title>
      ...
      <Modal show={editModalShow} onHide={handleEditModalClose}>
        <Modal.Header>
          <Modal.Title>{Trans("UPDATE_SETTING_GROUP_KEY", language)}</Modal.Title>
