...
import {
  webSettingGroupUrl,
  webSettingGroupChangeStatusUrl,
  languageDeleteUrl,
} from "config";
...
function Index() {
  ...
  const getData = (pagev, perPageItemv, searchData, sortBys, OrderBy) => {
    ...
    POST(webSettingGroupUrl, filterData)
      .then((response) => {
  ...
  const ChangeFunction = (id) => {
    ...
    POST(webSettingGroupChangeStatusUrl, editData)
      .then((response) => {
        const { message } = response.data;
        filterItem("refresh", "", "");
        Notify(true, Trans(message, language));
    ...
    <Content>
      <PageHeader
        breadcumbs={[
          ...
          {
            title: Trans("WEB_SETTING_GROUP", language),
            link: "",
            class: "active",
          },
        ]}
            ...
            <div className="card" id="custom-user-list">
              <div className="card-header pd-y-15 pd-x-20 d-flex align-items-center justify-content-between">
                <h6 className="tx-uppercase tx-semibold mg-b-0">
                  {Trans("WEB_SETTING_GROUP_LIST", language)}
                </h6>
                <div className="d-none d-md-flex">
                  ...
                  >
                    <Button variant="primary" onClick={handleModalShow}>
                      ...
                      />
                      {Trans("ADD_WEB_SETTING_GROUP", language)}
                    </Button>
                  </CheckPermission>
                </div>
              </div>
      ...
      <Modal show={show} onHide={handleModalClose}>
        <Modal.Header>
          <Modal.Title>{Trans("ADD_WEB_SETTING_GROUP", language)}</Modal.Title>
      ...
      <Modal show={editModalShow} onHide={handleEditModalClose}>
        <Modal.Header>
          <Modal.Title>
            {Trans("UPDATE_WEB_SETTING_GROUP", language)}
          </Modal.Title>
